...
public class Box implements Iterable<Ball> {
    ...
    public boolean add(Ball b) {
        if (currVol + b.getVolume() > maxVol) {
            return false;
        }
        boolean add = ballContainer.add(b);
        if (add) {
            currVol += b.getVolume();
        }
        return add;
    ...
    public Iterator<Ball> getBallsFromSmallest() {
        // add balls to list to sort
        List<Ball> list = new ArrayList<Ball>();
        for (Ball b : ballContainer) {
           list.add(b);
        }
        Collections.sort(list, new Ball(0));
//
//        BallContainer temp = new BallContainer();
//        for (Ball b : list) {
//            temp.add(b);
//        }
    ...
    public boolean remove(Ball b) {
        boolean remove = ballContainer.remove(b);
        if (remove) {
            currVol -= b.getVolume();
        }
        return remove;

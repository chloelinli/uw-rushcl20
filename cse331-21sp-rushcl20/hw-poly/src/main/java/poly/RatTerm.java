...
public final class RatTerm {
    ...
    public boolean isNaN() {
        return this.getCoeff().equals(RatNum.NaN);
    }
    ...
    public boolean isZero() {
        return this.getCoeff().equals(RatNum.ZERO);
    }
    ...
    public double eval(double d) {
        ...
        else if (this.equals(RatTerm.ZERO) && d == 0) {
            return 1.0;
        }

        return this.getCoeff().doubleValue() * java.lang.Math.pow(d, this.getExpt());
    }
    ...
    public RatTerm add(RatTerm arg) {
        ...
        else if (this.getExpt() != arg.getExpt() && !(this.isZero() || arg.isZero())) {
            throw new IllegalArgumentException("Cannot compute operation");
        }
        ...
        RatNum n = this.getCoeff().add(arg.getCoeff());
        return new RatTerm(n, exp);
    }
    ...
    public RatTerm mul(RatTerm arg) {
        ...
        RatNum n = this.getCoeff().mul(arg.getCoeff());
        int exp = this.getExpt() + arg.getExpt();
        return new RatTerm(n, exp);
    }
    ...
    public RatTerm div(RatTerm arg) {
        ...
        RatNum n = this.getCoeff().div(arg.getCoeff());
        int exp = this.getExpt() - arg.getExpt();
        return new RatTerm(n, exp);
    }
    ...
    public RatTerm differentiate() {
        ...
        else if (this.getExpt() == 0) {
            return new RatTerm(RatNum.ZERO, 0);
        }

        RatNum exp = new RatNum(this.getExpt());
        RatNum n = this.getCoeff().mul(exp);
        return new RatTerm(n, this.getExpt() - 1);
    }
    ...
    public RatTerm antiDifferentiate() {
        ...
        RatNum exp = new RatNum(this.getExpt());
        exp = exp.add(this.ONE);
        RatNum n = this.getCoeff().div(exp);
        return new RatTerm(n, this.getExpt() + 1);
    }
